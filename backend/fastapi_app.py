...
DEFAULT_MODEL_DIR = (
    ROOT_DIR / "saved models" / "hingmbert_bullysenti_epochs5_maxlen128_lr2e-5"
)
...
class HingmBertPredictor:
    def __init__(self, model_dir: Path) -> None:
        if IMPORT_ERROR is not None:
            raise RuntimeError(
                "Missing inference dependencies. Install backend/requirements-fastapi.txt first."
            ) from IMPORT_ERROR
    ...
    def predict(self, text: str) -> dict:
        ...
        prediction = "bullying" if predicted_label == "positive" else "clean"
...
def get_model_dir() -> Path:
    model_dir = Path(os.getenv("TOXISENSE_FASTAPI_MODEL_DIR", DEFAULT_MODEL_DIR))
    return model_dir.resolve()


def build_predictor() -> HingmBertPredictor:
    model_dir = get_model_dir()

    if not model_dir.exists():
        raise FileNotFoundError(f"Model directory not found: {model_dir}")

    return HingmBertPredictor(model_dir)


app = FastAPI(title="ToxiSense HingmBERT API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def predict(payload: PredictionRequest):
    ...
    result = heuristic_result if heuristic_result is not None else predictor.predict(normalized_text)
...
if __name__ == "__main__":
    ...
    host = os.getenv("TOXISENSE_FASTAPI_HOST", DEFAULT_HOST)
    port = int(os.getenv("TOXISENSE_FASTAPI_PORT", DEFAULT_PORT))
